...
Чтобы не приводил к регрессии 

[phpunit](https://phpunit.de) - это фреймворк для написания тестов.

## Версии phpunit

По историческим причинам версия phpunit напрямую зависит от версии php 
которая установлена у вас на сервере.

В момент написания этой статьи (ноябрь 2020) актуальными и поддерживаемыми являются версии phpunit 8 и 9.
...
не запустится. При написании тестов придерживайтесь одной мажорной версии.
...
Если он не установлен у вас на сервере, установим его.
...
После установки библиотека добавиться в файл `composer.json`
...
Запускаем так `composer phpunit`
...
Проверка установки последней версии фреймворка

```shell script
composer phpunit -- --check-version
```
...
Если phpunit нужен глобально для всей системы можно переместить файл в директорию со всеми скриптами
