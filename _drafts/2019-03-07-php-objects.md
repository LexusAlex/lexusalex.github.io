...
$o2 = new \oop\Application; // если конcтруктор не определен, то () можно опустить
...
Конструктор - это специальная функция которая автоматически вызывается при создании объекта, она не должна ничего возвращать.
В конструктор передаются параметры которыми нужно инициализировать объект

Добавим функцию конструктора в класс `Student`
...
NOW;
    ...
    {
        $this->age = $age; // $this к контексте класса это ссылка на сам создаваемый объект
        $this->course = $course;
        $this->desc = $desc;
    }
}
...
~~~
Код инициализации объектов `Student` можно сократить и переписать таким образом

~~~php
<?php
$student1 = new \oop\Student(24, 1, "Студент Вася");
$student2 = new \oop\Student(26, 3, "Студент Петя");
~~~

> Методы которые начинаются с __ именуются магическими и должны быть объявлены как public

Основная задача конструктора заполнить объект переданными значениями

Следующий магический метод называется деструктор. Деструктор вызывается автоматически когда будет завершена работа
скрипта или будут удалены все ссылки на объект.

На практике необходимости использования деструкторов бывает крайне редко
...
### Как правильно обращаться к свойствам объекта

Есть такой термин как инкапсуляция
