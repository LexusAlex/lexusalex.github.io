...
2. Суперблок - единичный блок который содержи метаданные файловой системы.
3. Таблица индексных дескрипторов - список файлов и информация о них.
...
```
>> В расширяемом разделе диска не рекомендуется создавать файловую систему.
...
Информацию о файловой сиcтеме, суперблоке, группах блоках можно получить с помощью команды `dumpe2fs /dev/sda1`

### Таблица индексных дескрипторов

inode (индексный дескриптор) - структура данных в которой хранится метаинформация о файлах,
каталогах и других объектах, кроме данных и имени файла.

Каждый фаил в файловой системе имеет свой уникальный индексный дексриптор или i-node.
...
Приминимо к файловой системе ext2 каждый индексный дескриптор содержит 15 указателей.
c 0 по 11 указывают на положение 12 блоков в файловой системе. 13 блок содержит список указателей.
...
Другие файловые системы монтируются в корневой каталог. Для монтирования файловых систем использутеся команда `mount`, а
для размонтирования `umount`.

### Тип фаил

Теперь мы плавно подошли к основополагающему понятию фаил - это именованная область данных на внешнем носителе
