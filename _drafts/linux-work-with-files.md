...
Локально пользователи хранятся в двух таблицах, где каждый пользователь представлен одной строкой с разделителями:
1. `/etc/passwd` - системный файл паролей
2. `/etc/shadow` - теневой файл паролей

Каждый пользователь имеет уникальное имя для входа, числовой идентификатор (UID), групповой идентификатор (GID)
